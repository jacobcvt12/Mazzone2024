...
lucas473 <- read_csv("~/Delfi/GitRepos/reproduce_lucas_wflow/data/lucas_5mbs_delfi473.csv")

clin <- read_excel("~/Delfi/GitRepos/reproduce_lucas_wflow/data/LUCAS_metadata.xlsx") %>%
  filter(Training == "YES")

res <- lucas473 %>%
  inner_join(clin) %>%
  mutate(ratio=short.cor/long.cor,
         group=ifelse(type == "cancer", "Lung cancer", "Non-cancer"),
         group=factor(group, levels=c("Non-cancer",
                                      "Lung cancer"))) %>%
  group_by(id) %>%
  mutate(centeredslratio=ratio-mean(ratio)) %>%
  ungroup() %>%
  select(id, bin, armlevel=arm, value=centeredslratio, group) %>%
  filter(!str_detect(armlevel, "X")) %>%
...
pdf("supplemental_figure_lucas2a.pdf", height=3.25, width=7.08661)
fig 
dev.off()
